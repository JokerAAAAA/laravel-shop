    ...
    public function handle()
    {
        ...
        foreach ($installment->items as $item) {
            ...
            try {
                ...
                \Log::warning(
                    '分期退款失败：'.$exception->getMessage(),
                    [
                        'installment_item_id' => $item->id,
                    ]
                );
                // 加入某个还款计划退款报错了，则暂时跳过，继续处理下一个还款计划的退款
                continue;
            }
        }
        ...
        switch ($item->payment_method) {
            case 'wechatpay':
                app('wechatpay')->refund(
                    [
                        ...
                        // 微信支付的退款结果并不是实时返回，而是通过退款回调来通知，因此这里需要配上退款回调接口地址
            ...
            case 'alipay':
                $result = app('alipay')->refund(
                    [
                        'trade_no' => $item->payment_no,   // 使用支付宝交易号退款
